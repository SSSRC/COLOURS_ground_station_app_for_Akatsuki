...
function createWindow(): void {
  mainWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      sandbox: false, // ★ セキュリティブロックを解除
    },
  });

  // dist フォルダから見て1つ上の階層にある index.html を読み込む
...
function parseTelemetryLine(line: string): TelemetryPacket | null {
  ...
  if (Number.isNaN(timeMs) || Number.isNaN(phase)) return null;
  ...
  if (pressureSamples.length === 0) return null;

  return { timeMs, phase, pressureSamples, temperatureC, raw: trimmed };
}
...
app.whenReady().then(() => {
  ...
  ipcMain.handle("serial:connect", async (_event: IpcMainInvokeEvent, config: SerialConnectConfig) => {
    try {
  ...
  ipcMain.handle("serial:send-sequence", async (_event: IpcMainInvokeEvent, sequenceNo: number) => {
    try {
      if (!port || !port.isOpen) return { ok: false, message: "serial port is not open" };
